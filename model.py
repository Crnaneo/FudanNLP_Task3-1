...
class Model(nn.Module):
    def __init__(self, embedding, dim=128, n_head=8, num_layers=3,hdim=2048, *args, **kwargs):
        ...
        self.fc_out = nn.Linear(dim,len(self.embedding.words))
    def forward(self,src_tokens,tgt_tokens):
        src_emb = self.embedding(src_tokens);
        tgt_emb = self.embedding(tgt_tokens);
        tgt_mask = self.transformer.generate_square_subsequent_mask(tgt_tokens.size(1)).to(tgt_tokens.device)
...
class TransformerByHand(nn.Module):
    def __init__(self,embedding,dim=128,n_head=8,num_layers=3,hdim=2048, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.embedding = embedding;
        self.dim = dim;
    ...
    def forward(self,src_tokens,tgt_tokens):
        ...
        src_pad_mask = (src_tokens == 0)
        tgt_pad_mask = (tgt_tokens == 0)
...
class DecoderOnly(nn.Module):
    def __init__(self, embedding, dim=128, n_head=8, num_layers=3,hdim=2048, *args, **kwargs):
        ...
        self.fc_out = nn.Linear(self.dim,len(self.embedding.words));
